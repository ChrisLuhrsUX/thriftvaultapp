...
### Session, 2026-05-18
- **NEW `ItemCategory = 'watches'` + WATCH BRAND HARD RULE + COMMON HALLUCINATION TRAPS watch entry** (`types/inventory.ts`, `app/(tabs)/index.tsx`, `services/gemini.ts`). Trigger: gold-tone chronograph scan came back named "ZUNAIR Gold-Tone Chronograph Watch" priced $25-$65; "ZUNAIR" is a watermark / text overlay on the watch box or band area, not a watch brand. Existing prompt scoped WATCHES sub-tier to "visible brand mark on dial AND matching case-back signal" but the `name` field upstream pulled brand from any in-frame text. Fixes: (1) added `'watches'` to ItemCategory union + ITEM_CATEGORIES array, between `'accessories'` and `'furniture'`; (2) added Watches filter chip to CATEGORY_GROUPS in Vault; (3) synonym map flipped watch/watches/wristwatch/chronograph from `'accessories'` to `'watches'`; (4) prompt category guidance line added for `category "watches"` covering wristwatch / pocket watch / smartwatch when the watch is primary subject; (5) JEWELRY PRICING routing broadened to fire on `category === 'watches' OR (category === 'accessories' AND jewelry keyword)`; (6) NEW WATCH BRAND HARD RULE mirroring BAG AUTHENTICATION format: brand must come from a logo/wordmark on the DIAL, a maker's stamp on the CASE BACK, or a signed crown (Rolex/Omega/AP/Cartier/Patek monogram). Explicit rejections: text on the watch BOX or packaging, watermarks/photo overlays/social-media handles, decorative band/bracelet engravings (often jeweler or owner monograms), case material color, bezel silhouette, strap material. Without dial-or-case evidence, name by silhouette + case material + complication and price at Fashion tier $20-$80; (7) NEW COMMON HALLUCINATION TRAPS WATCH entry listing octagonal bezel (NOT AP Royal Oak without dial), cyclops + round case (NOT Rolex without crown), Roman-numeral dial (NOT Cartier Tank without wordmark), moon-phase subdials (NOT Patek/Vacheron/JLC), diver bezel (NOT Omega without dial), three-subdial chronograph (NOT Speedmaster/Carrera without dial), and the box/band/watermark rejection. No migration: pre-launch, existing accessories items stay where they are. PRICING_TIERS.md rows 082 (Watches Fashion) and 083 (Watches Luxury) unchanged. **Pattern lesson: when a category has its own resale dynamics (dedicated marketplace, distinct authentication pattern, wide price-tier spread), promote it to its own `ItemCategory` rather than keeping it under a generic umbrella; the dedicated category lets routing rules gate precisely AND lets the user filter without keyword friction. Same shape as the Vintage Tableware tier 088 sitting in `furniture` for routing purposes but treated as its own pricing branch.**
- **NEW pricing tier 089, Modern Mass-Market Functional Small Furniture + CONDITION REPORTING HARD RULE + FURNITURE COMPARTMENT NAMING HARD RULE + Clear-button reposition** (`services/gemini.ts`, `PRICING_TIERS.md`, `app/(tabs)/scan.tsx`). Furniture scan of a blurry dark-stained pine chairside table (with magazine-rack slot above lower shelf) priced $40-$100 / baseline $60 via the Generic vintage no-brand solid-wood $40-$300 tier; real Facebook Marketplace comps $15-$30 as-is or $50-$80 refinished. External Gemini review flagged four issues: pricing too ambitious, "good condition" description ignoring visible top-right edge chipping, lumped "Drawer and Shelf" missing the magazine rack slot entirely, and Clear-button UI overlapping the bottom-drawer area in the camera box. Fixes: (1) NEW tier 089 for modern mass-market functional small furniture (side tables, nightstands, small bookshelves, TV stands, small cabinets sold new at Walmart/Target/Wayfair budget/Amazon for $40-$120) at $15-$50 used / $40-$80 refinished, with a ROUTING gate requiring at least ONE positive vintage tell (hand-cut dovetails, mortise-and-tenon, real solid grain at unfinished interior, manufacturer plate from 60s-70s or earlier, antique-mall finish patina, weight commensurate with size) before escaping to the vintage tier. (2) NEW CONDITION REPORTING HARD RULE: description MUST name every visible defect (chipped edges, finish loss, scratches, dents, water rings, sun fade, drawer sag, etc.) with location; "good condition" allowed ONLY when zero defects visible; pricing must reflect visible condition (drop to lower tier portion or next tier down). (3) NEW FURNITURE COMPARTMENT NAMING HARD RULE: name each compartment by type (drawer / shelf / cubby / magazine rack slot / door cabinet) AND position (top/middle/bottom); do not lump multiple compartments under a single label. (4) Clear-button repositioned from center-bottom to top-right of the camera box (`clearBtnBlur` style: `bottom: 16, alignSelf: 'center'` → `top: 16, right: 16`) so it no longer overlaps the scanned subject. **Pattern lesson: visual styling of "dark wood + small + traditional shape" is NOT a vintage tell on its own; modern factories produce exactly this look at Walmart prices. Tier routing must gate on positive vintage construction tells, not aesthetic appearance. Same pattern shape as the 5/17 record-player MDF-veneer trap.**
- **PHOTO CAPTURE ARTIFACTS EXEMPTION + REAL TEXTILE/TRIM EXEMPTION on AI-GENERATED PHOTO branch** (`services/gemini.ts:471`). Three false-positive red flags on real photos: (1) decora/scene mirror-selfie with rainbow hair + GIR character tee + bloom from window light + motion blur on hands, (2) similar jirai/decora full-body selfie with kuromi-style face filter + striped tee + plushie-cluttered background, (3) studio lingerie shot with Victorian-floral pink lace corset over sheer mesh bodysuit. Photos 1 and 2 likely tripped diffusion/smearing (motion blur on hands), garment edge-bleed (bloom around hair/clothes), face/skin/hair render compound (filter smoothing + bloom-faded hair edges), and SIGNATURE A mirror-selfie composite (the cluttered subculture bedroom was getting read as "styled composition"). Photo 3 likely tripped wobbly factory print (intricate lace pattern misread as AI-drawn linework) and lookbook-styling amplifier. NEW PHOTO CAPTURE ARTIFACTS EXEMPTION covers motion blur on extremities, bloom/halation/lens-flare from bright sources, JPEG/video/TikTok-screenshot compression banding/posterization, low-light noise, out-of-focus shots, beauty-filter softness, Y2K/dreamy/grunge/film filter aesthetics, and explicitly carves out alt-fashion/J-fashion subculture styling (decora/scene/fairy kei/jirai kei/lolita/harajuku/kidcore) including rainbow hair, plush-cluttered bedrooms, kandi accessories, fishnets, leg warmers, character-print tees (GIR / Hello Kitty / My Melody / Kuromi / Domo) as REAL fashion and a NEGATIVE signal against SIGNATURE A/B "perfectly clean room" patterns. NEW REAL TEXTILE/TRIM EXEMPTION covers lace (Chantilly, Alençon, eyelash, broderie anglaise, guipure, illusion bridal mesh, baroque/Victorian floral, scalloped, crochet), machine-embroidered overlay (bridal/prom/occasion-wear), beaded/sequined/pearl/rhinestone appliqué, jacquard/brocade/damask/tapestry weaves, sheer/mesh/tulle/organza, and tie-dye/ombre/batik. Both exemptions sit BEFORE AI CONTEXT RECOGNITION so they short-circuit signature-matching when capture artifacts or intricate textiles are the only "AI tells" present. Filter-softened skin alone no longer trips the face/skin/hair compound; that pair now requires hair-edge fade AND one unrelated AI tell. **Pattern lesson: AI-detection rules need explicit carve-outs for the optical artifacts of real cameras and the intricate-but-mechanical patterns of real textiles. Without these, the rule treats every real-world quirk (motion blur, bloom, compression banding, lace) as an AI tell. Carve-outs go BEFORE the signature/tell pipeline because once a signature matches, the pairing threshold drops and the carve-out is moot.**
- **NEW pricing tier 088, Vintage Tableware / Dinnerware / Collector Plates + sibling TABLEWARE BACKSTAMP & ERA HARD RULE** (`services/gemini.ts`, `PRICING_TIERS.md`). Vintage plates were falling through the Decor smalls $10–$80 unbranded / $40–$300 signed studio pottery generic bucket and being severely underpriced. Real Depop/Etsy/eBay sold comps last 90 days: Pyrex Butterprint $80–$200, Gooseberry pink $120–$300, grail patterns (Lucky in Love, Eyes, Hot Air Balloons) $200–$1500+; original-era Fiestaware medium green plate $80–$400, rare-color Disc Water Jug $150–$800; Roseville Pine Cone vase $80–$300, rare Roseville (Della Robbia, Rozane) $200–$1500+; Stig Lindberg Bersa / Arabia Paratiisi / Dansk by Cyrén signed pieces $80–$300+; Boleslawiec dinner set $80–$300. New tier covers Pyrex (Corning 1947–80s opal-glass), original Fiestaware (Homer Laughlin pre-1986), Wedgwood Jasperware vintage, Spode (Blue Italian / Christmas Tree), Noritake Art Deco "M-Mark" Morimura, American art pottery (Roseville / McCoy / Hall), Royal Doulton HN figurines, Scandinavian MCM (Arabia / Iittala / Dansk / Gustavsberg + designer signatures), Depression / Carnival / Milk glass, and Polish Boleslawiec / Mexican Talavera / Italian majolica. TABLEWARE BACKSTAMP & ERA HARD RULE mirrors BAG AUTHENTICATION format: requires positive maker's-mark identification before vintage premium pricing; calls out post-1986 Fiestaware trap colors (periwinkle, sapphire, plum, raspberry, lemongrass, sunflower, persimmon, scarlet, tangerine, peacock, cinnabar, paprika, marigold, twilight, sage, meadow, daffodil, lapis, lilac, claret) that did not exist 1936–1972 to keep reissues out of the vintage band; vintage Pyrex requires opal/milk-glass body (modern Instant Brands is clear borosilicate); Roseville reproductions named via crisp modern lettering vs worn impressed marks. Condition penalties baked into the rule: chip/crack/repair −40–60%, crazing −20–35% (acceptable on art pottery, value-tanking on dinnerware), glaze loss −25–40%, missing set pieces → price per-piece only. Decor smalls line narrowed from generic "ceramics" to "non-functional decorative ceramics" to remove routing ambiguity between functional dinnerware (tier 088) and decorative pottery (tier 073). Partially closes POST_LAUNCH "Pottery & small craftsmanship" backlog; remaining gaps are signed grail-tier studio pottery (Lucie Rie / Hans Coper / Voulkos / Takaezu / Beatrice Wood) above the current $40–$300 ceiling, dedicated `ItemCategory = 'pottery'` for vault grouping, photo-detection clamp for damage signals, and a `POTTERY EXCEPTION` so the handmade labor formula does not absurd-price a $20 hobbyist mug as a custom commission. **Pattern lesson: when a tier's most valuable sub-segment depends on a specific maker's mark, the auth rule must name the era markers (mold marks, date codes, color palettes) that distinguish vintage from reissue. The post-86 Fiestaware trap-color list is the same shape as the modern Pyrex reissue trap and the modern Wedgwood Jasperware trap, name the era markers explicitly because the model otherwise reads "Fiesta" as "vintage Fiesta".**
...
### Session, 2026-05-14
- **AI detection diagnostic instrumentation, parked** (`services/gemini.ts`). 3 `__DEV__`-gated log points (raw text + `redFlags` regex slice + parsed array) in `callGemini`/`callAnthropic`/`runScanPipeline` + `EXPO_PUBLIC_FORCE_CLAUDE_SCAN=1` bypass at top of `callWithFallback`. User decided "we're not debugging anymore" after instrumentation landed. STATUS: stripped 5/17.
- **`hooks/usePurchases.ts` TS errors suppressed**. Two `@ts-expect-error` on static type ref + dynamic `import('react-native-purchases')` (package uninstalled until prebuild). Will fail once installed, signaling removal.
- **Pricing-drift watch basket extended to 14 tiers** (`trig_0113xK23HSeSpB46ySDYJtBF`). STATUS: SUPERSEDED 5/16 by 85-tier registry. Added (13) Lamps (generic $25–$120 / vintage brass/Stiffel/MCM ceramic $40–$200 / Tiffany authenticated $500–$5000+ / Tiffany-style repro $40–$150) and (14) Mass-market particleboard (IKEA / Wayfair budget / Target Threshold, $15–$120). Furniture rows substitute eBay completed-sold + Chairish + 1stDibs + Etsy (Depop/Poshmark/FBM excluded).
- **Legal docs at pretty URLs**: `terms/index.html` and `privacy-policy/index.html`. Canonical `thriftvaultapp.com/terms/` and `/privacy-policy/`. In-app refs updated: `PaywallModal.tsx:209,213`, `app/(tabs)/profile.tsx:120,124`, `STORE_LISTING.md:29`, `MVP.md:40` (off legacy `chrisluhrsux.github.io/thriftvaultapp/assets/...`).
- **Kit waitlist form on landing page hero** (`index.html`, Form ID 9443345). Direct AJAX `fetch()` to Kit's `/subscriptions` endpoint, vanilla submit handler, custom markup (cream-warm input, brass-soft border, teal CTA). No Kit JS/CSS dependency. Success: "You're on the list. Check your inbox (and spam folder) to confirm." Kit dashboard "Subscribers" view defaults to confirmed-only; unconfirmed appear elsewhere (easy to misread as failed submission). **Pattern lesson: static-site form integrations on GH Pages, skip the provider's JS/CSS and POST directly to their endpoint with custom markup, cleaner design and no class-name CSS leak.** **Pattern lesson: brand-new Kit accounts hit spam routinely until sender reputation warms (~25–50 confirmed subs). Mention spam in success message.**
- **Hangtag design language extended page-wide** (`index.html`). Pattern: punched-hole + twine top + brass-soft border + `4px 4px 14px 14px` radius + alternating 0.3–0.5° tilts that straighten on hover with `translateY(-4px)` lift. Applied to pricing cards (3-col), features (3x2 grid), hero logo. Brass Save 33% / Save 50% ribbons (6° tilt, top-right) on Season Pass + Annual. **Pattern lessons:** (1) vintage motif can become page-wide visual language when applied consistently across card content, hangtag fit because the product is literally a thrift app; (2) dropping decoy "featured" emphasis (per 5/11) leaves cards equal, savings ribbons backfill the hierarchy gap with truthful value signaling, visual emphasis follows actual value.
- **Site copy warm-up sweep** (`index.html`). Replaced corporate phrasing across meta description, hero lead, feature bodies, plans intro, effective texts, trial strip, about, contact eyebrow, prelaunch bar. Removed "AI scan companion" (3 spots).
- **AI estimate disclosure surfaces**. Onboarding slide 1 italic mauve note ("Estimates aren't promises. Double-check sold comps before buying or selling, especially for high-value items"). Profile Settings "About AI estimates" row (sparkles icon) opens `Alert.alert` (Depop/eBay/Poshmark sold comps as starting point, range width = confidence, verify manually for high-value or low-data items). Scan card unchanged (existing range + Yes/No + "Low resale data" + red flag banners communicate uncertainty contextually). **Pattern lesson: per-scan disclaimer dilutes existing uncertainty signals. One-time onboarding mention + permanent Profile section is honest hygiene without visible nag.**
- **TikTok series brand = "Best Thrift Flips"**. #thriftflip dominates TikTok algorithm vs #upcycle (crafty/Etsy lean), aligns with "thrift" in app name. `LAUNCH_OPS.md` gained "Social content backlog (pre-launch)" section.
- **Trial strip color iteration**, settled on `--teal-deep` (was `--charcoal`, briefly `--brass`). Brass + cream fails WCAG AA (~2.5:1); brass + charcoal passes but reads washed. Teal-deep ties to H1 italic + Notify Me button.
- **Coming-soon hero badge removed**. Pill's narrow content-hugged width mismatched the wider form below; prelaunch bar already conveys "coming soon". **Pattern lesson: stacked elements with mismatched widths feel "off". Either align widths or drop one element, don't try to bridge with margin.**
- **Feature card 1–6 numbering dropped** (user: tacky). Hangtag motif carries the rhythm.

### Session, 2026-05-13
- **AI-GENERATED ARTWORK false positive on plain ripped denim** (`services/gemini.ts`). Branch firing on DIY-distressed jeans with no print/graphic. Fixes: (1) PRECONDITION at top of branch requires actual applied graphic design (paint, sublimation, screen print, embroidered pictorial); exempts plain solids, distressing/rips/bleaching, classic textile repeats, construction details, brand labels. (2) Dropped loose "Photorealistic art style with no visible artist credit" tell. (3) Re-gated "ERR ON THE SIDE OF FLAGGING" behind precondition.
- **Before/After false positive on denim back-view** (`services/gemini.ts` `multiPhotoSuffix`). 3-photo front-rip jeans + back view was forcing `beforeAfterDetected = true` because back view trivially satisfies "lacks hand-applied elements" + "factory garment intact" (rips are front-loaded). Added denim back-view clause to "Tells AGAINST (b)" + anatomical-concentration principle (front-only graphics, knee rips, back-yoke embroidery, single-sleeve paint are one-sided design, not a before).
- **Repurposed-textile outerwear underpricing** (`services/gemini.ts` PROMPT + `HANDMADE_SUFFIX`). Same sherpa-lined blanket-jacket scanned twice: $135 then $55. HANDMADE OUTERWEAR EXCEPTION split into (i) FIBER-ART (unchanged) and (ii) REPURPOSED-TEXTILE/SEWN-FABRIC (blanket-jacket, quilt-coat, tablecloth-blazer, curtain-duster, frankenjacket). (ii) minimum MODERATE ($55–$120), complex ($90–$180) when lined/fully-finished. FIRST-PASS ANCHORING relaxed to band mid-to-upper for (ii). Sherpa-lined plaid blanket-jacket = COMPLEX, anchor $130–$160. **Pattern lesson: handmade pricing exceptions need sub-tracks by construction type, not just complexity; fiber-art and sewn-fabric have different labor floors and comp clusters.**
- **Landing page features rewrite** (`index.html`). Stripped jargon: "multimodal pricing", "tier-aware rounding", "unknown-maker comps", "authentication tells", "sold-through tally". Aligned titles with body copy. Added feature 6 "Yours, on your phone" (local-only privacy differentiator). Reworded "blanket-jackets" → "upcycles".
- **Em dash sweep + memory** (`feedback_no_em_dashes.md`). Swept 32 tracked files (literal em-dash → commas, en dashes in `$130–$160` ranges preserved) + 4 `&mdash;` HTML entities in `index.html` that literal-character grep missed. Memory entry includes HTML-entity gotcha.
- **LOLITA/KAWAII J-FASHION carve-out on ALL-OVER DIGITAL PRINT** (`services/gemini.ts`). Carousel-print Angelic Pretty/BtSSB OP false-flag. 4th "Do NOT flag" clause: lolita silhouette (puff/mutton/bishop sleeves + 2 of tiered ruffled skirt / multi-row lace / ribbon bows / pintucks/smocking / cinched waist / shirred back / matching headdress) is the firewall; elaborate pictorial sublimation (carousels, sweets, fairytale castles, Alice, stained glass, crowns, music notes, libraries, celestial, heraldic roses, nostalgic toys) is the defining genre aesthetic. Covers AP/BtSSB/AatP/IW/Meta/Victorian Maiden/Mary Magdalene/Putumayo/h.NAOTO/ETC/Jane Marple + replica/inspired-by. Lolita is factory-made (`isCustom = false`). Out of scope: dedicated lolita pricing tier ($150-$800 brand pieces underprice via generic dresses) + replica detection (TaoBao AP print-fingerprinting needs database).
- **AI photo detection iteration did NOT land, deferred to 5/14** (`services/gemini.ts`). 3 AI images (2-panel mirror-selfie cardigan composite; flat-lay mint sweatshirt with wobbly embroidered face; flat-lay striped speckled shirt with mismatched buttons + garbled tag) bypassed 4 successive edits. STATUS: 5/14 instrumentation parked, 5/17 stripped. Edits STILL IN PLACE: (1) 4 paired tells (multi-panel composite drift, mirror-selfie generic phone case, face/skin/hair render, lookbook amplifier); (2) 4 more (wobbly print/embroidery, hardware inconsistency, flat-lay AI cleanliness, adjacent-item consistency); (3) AI CONTEXT RECOGNITION signatures A (mirror-selfie composite) / B (flat-lay listing) / C (sketch-graphic), pairing threshold = 1 soft tell with signature match, narrowed `:431` professional-photo exemption to require REAL brand-origin signals (watermark/runway/magazine grain/retailer chrome), bias line acknowledges 2026 AI prevalence (Midjourney/SDXL/FLUX/Sora/Reve), threshold = 1 hard OR 2 soft OR 1 soft + signature; (4) AI-GENERATED ARTWORK gained 2 tells (wobbly outline lines, incomplete/floating features) + "cute/sketch/cartoon style is not automatic intentional style" charity-defeater.

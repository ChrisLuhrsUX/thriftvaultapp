...
- **detail.tsx auth separator bug** — `insightsAuthSection` was using `borderBottomWidth` while upcycle/history/delete all used `borderTopWidth`. When auth sat between other sections it created a missing separator above auth and a doubled line below. Fix: moved border to `insightsAuthHeader` as `borderTopWidth` + `paddingVertical` + `minHeight` (matching upcycle pattern), deleted the now-empty `insightsAuthSection` style.
- **detail.tsx "Listing suggestions" separator** — `insightsIdeas` had no top border, so scan.tsx showed a separator above Listing suggestions but detail.tsx didn't. Added `borderTopWidth: 1, borderTopColor: surfaceVariant, paddingTop: md` to match.
- **scan.tsx upcycle bottom separator** — after the reorder, upcycle became the last section before `resultActions`. Added `borderBottomWidth` back to `upcycleSection` so there's a separator under the upcycle block.

...
- **Background scan** — iOS suspends network ~5–30s after backgrounding. `app/(tabs)/scan.tsx` AppState handler + `handleScanStaged` finally make the failure path *look* seamless (continuous spinner, no false toast, retry threads in on resume) but the scan still re-runs when iOS kills the network. Real fix is a `UIApplication.beginBackgroundTask` Expo native module (NOT "Background Fetch entitlement" — different iOS API for periodic fetches). Deferred to `POST_LAUNCH.md` because it requires prebuild → ends Expo Go workflow.
...
- **Apple Developer** — Individual enrollment active ($99/yr). Individual → Org conversion requested; awaiting Apple's next step. D-U-N-S, team ID, and account email tracked outside the repo.
...
### Session — 2026-05-03
- **Rescan dropping `beforeAfterDetected` pill** (`services/gemini.ts:499`, `app/detail.tsx:444`, `app/(tabs)/scan.tsx:1538`) — Clamp `parsed.beforeAfterDetected === true && images.length >= 2` was zeroing the verdict on every rescan because all four rescan paths send only the cover photo. Fix: on single-photo scans, fall back to `priorResult?.beforeAfterDetected === true` (which was set with multi-photo evidence). Covers both `rescanWrong` paths automatically since they thread `priorResult`. The two `confirmHandmade` paths intentionally do NOT pass `priorResult` (would mis-suffix the prompt as "wrong scan") — they OR in the prior verdict at the call site instead. The 4/30 session note claimed `RESCAN_CORRECTION_SUFFIX` already preserved the verdict; that was wrong — the prompt carried it but the post-process clamp dropped it.
- **Handmade sewn-fabric tops overpriced at $95–$180** (`services/gemini.ts` `HANDMADE_SUFFIX` + `runScanPipeline` clamp ~line 527) — User reported a from-scratch handmade satin V-neck top with lace trim returning $95–$180. Real Depop/Etsy comp range $40–$80. Confirmed broader pattern across handmade sewn tops. Root cause: 4/27 fix added uniform $180 ceiling for ALL handmade tops, but sewn-fabric DIY hobbyist work doesn't price at the labor-formula level — Depop buyers price by finished look because the market is saturated with TikTok tutorials. Crochet/hand-knit/cottagecore legitimately commands $80–$180 (visible labor + trending tier). Fix mirrors DENIM EXCEPTION pattern: NEW `HANDMADE SEWN-FABRIC TOP EXCEPTION` prompt clause abandons labor formula for finished-look tiers (simple $25–$55 / moderate $40–$85 / complex $60–$120) with material-term unlock vocabulary. Renamed `isAlteredTop` → `isHandmadeTop` and made it material-tier-aware: `isCrochetKnitText` regex routes to $180 cap / $30 floor; default sewn-fabric path gets $120 cap / $25 floor. `isExceptionalDenim` override still wins ($300). Trade-off: altered factory-base "premium streetwear" tops cap at $120 instead of prompt's $130 when no crochet keyword fires — minor regression, AI rarely overshoots prompt anyway.
- **"Knit" disambiguation** (`services/gemini.ts:530` + HANDMADE_SUFFIX) — Same olive top rescanned to $105–$180 because Gemini wrote "stretchy knit with lace trim" in `sub`. Initial regex matched bare `knit(ted)?`, routing factory knit fabric (jersey/spandex/ponte) to the $180 craft tier. Bare "knit" / "knitted" describes FABRIC TYPE in fashion vocab, not yarn craft — the trending tier needs explicit handmade-craft signals. Tightened regex to require `hand[-\s]?knit(ted)?` (or `crochet`, `knitwear`, `yarn`, `cottagecore`, `milkmaid`, `mending`, `patchwork`, `embroidered`, `macrame`, `needlepoint`). Added `knit fabric, stretchy knit, ribbed knit, ponte` to the SEWN-FABRIC EXCEPTION material list and unlock vocabulary. Added explicit NOTE in HANDMADE TOP CEILING distinguishing the two senses of "knit". Pattern lesson: ambiguous textile terms (knit, woven, jersey) need negative + positive disambiguation in both prompt and code regex, since fashion vocab overloads them.
- **Sentry DSN wired** (`.env` `EXPO_PUBLIC_SENTRY_DSN`) — TestFlight cut from launch plan (insufficient tester pool to extract value; recruiting external testers requires marketing reach not available solo). Sentry becomes the load-bearing crash signal once the app hits the public App Store. DSN pasted into `.env`, Metro restarted, verified with a temp `Sentry.captureException` row on profile screen (added + removed in same session). All other Sentry wiring was already in place from 4/19: `@sentry/react-native` 7.2.0, `Sentry.init` in `_layout.tsx` with `sendDefaultPii: false` + breadcrumb redactor, `metro.config.js` ESM fix, `app.json` plugin. Native crash reporting still requires `expo prebuild` (deferred — Expo Go workflow stays for now). Pre-launch SECURITY_AUDIT.md / LAUNCH_OPS.md "before TestFlight" items now read as "before App Store submit" — see `memory/project_launch_no_testflight.md`.
- **Scan background reverted to v1 logo** (`app/(tabs)/scan.tsx:117`) — `SCAN_BG_SOURCE` back to `thriftvault_logo.jpg` after the 5/01 v2 swap. v2 PNG is still in place across `app.json` icon/splash/Android/web favicon paths and `WebSidebar.tsx`; only the scan background prefers the JPG variant.

### Session — 2026-05-02
- **Boost-stacking guard for factory items** (`services/gemini.ts`) — Closes UX_AUDIT 4/26 #3 (5 unfixed pricing risks → 2 left). Load-bearing post-process clamp catches the compound-boost case (era × embellishment × denim_spike × trend × collab). New module-level consts: `BOOST_BUCKETS` (5 bucket regexes — era, embellishment, trend, collab, denim_spike) and `LUXURY_EXEMPT_RX` (luxury houses, designer collabs, vintage Levi's selvedge tier, NWT). Clamp at line 570 mirrors altered-base pattern: `!isCustomScan && !LUXURY_EXEMPT && boostCount >= 3` → cap `resaleHigh` to per-category ceiling (denim $180, tops/dresses $130, bottoms $120, outerwear $200, shoes $200, bags $220, accessories $120) with proportional `resaleLow` scale and $15 floor. Custom items skip (own clamps). Worst-case before fix: vintage Y2K Diesel rhinestone flare jeans → $172–$1,290 (7× over comp); after: clamped to $180. Prompt-side `BOOST STACKING — HARD RULE` line inserted between `Trend premiums` and `Q2 2026 ACTIVE SPIKES` as belt-and-suspenders. `HANDMADE_SUFFIX` intentionally untouched (custom labor formula is dominant path; risks confusing the model).
- **Confidence tied to range width** (`services/gemini.ts:33`) — Closes UX_AUDIT 4/26 #5. New `confidenceFromRangeWidth(aiConf, low, high)` helper, downgrade-only. Ratio thresholds anchored to actual prompt tier widths: `≤3.0×` no change (common factory `$25–$80` is 3.2×); `>3.0×` caps `high → medium`; `>4.0×` caps to `low`. Wraps the inline confidence parse in the return object. `low`/`medium` never upgrade; `low ≤ 0` no-op. Prevents the `$50–$1200 "high confidence"` lie when boost stacking or sparse comps blow out the range.
- **Category update on rescanWrong** (`app/detail.tsx:520`) — Wrong-scan rescan was writing `name` and `resale` to the Item record but ignoring `result.category`, even though the rescan prompt explicitly tells Gemini to reconsider miscategorization. Added `catUpdate = result.category && result.category !== 'other' ? { cat: result.category } : {}` alongside existing `nameUpdate`. Guard against `'other'` prevents downgrading a specific prior category to the AI's "unsure" fallback. Handmade-confirm path (line 454) intentionally untouched — that signal is "wrong pricing tier", not "wrong item."
- **"Size guess" placeholder bug** (`services/gemini.ts:41`) — Schema example `"sub": "Brief description (size guess, color, material, condition)"` was making Gemini echo the literal phrase "Women's size guess" when it couldn't estimate the size (other fields filled correctly). Rewrote to `"Brief description: estimated size if visible (e.g. \"Women's 8\", \"Men's L\", \"US 10\"), color, material, condition. Omit any field that can't be determined from the photo — never echo the field name as a placeholder."` Examples disambiguate "size" as a value not a word; explicit omit-instead-of-echo closes the loophole.
...
#### Cursor
- **Security audit + fixes** (new `SECURITY_AUDIT.md`; `eas.json`, `CLAUDE.md`, `SAFETY.md`, `.claude/settings.json`, `app/_layout.tsx`) — Repo public, 0 audience. Scrubbed personal email from `eas.json` `submit.production.ios.appleId` and D-U-N-S/Apple-support correspondence from CLAUDE/SAFETY (HEAD only — old commits retain leaks; full removal needs repo-private or history rewrite). Hardened `Sentry.init` with explicit `sendDefaultPii: false` + `beforeBreadcrumb` redactor on `ui.input`/`ui.click` (keeps user-typed notes/store/name out of crash reports). Closed `.claude/settings.json` package-manager loophole — added `pnpm`/`pnpx`/`yarn`/`yarn dlx` deny matchers for `expo prebuild` and every destructive migration tool (prior coverage was `npx`/`bunx` only). Open user actions tracked in `SECURITY_AUDIT.md`: repo visibility, Anthropic per-key spend cap (no bundle-ID restriction available), Gemini bundle-ID restriction verification, wire `EXPO_PUBLIC_SENTRY_DSN`.
- **`SECURITY_AUDIT.md`** (new) — point-in-time snapshot + 10-step reusable methodology to rerun before each major release. Cross-linked from `SAFETY.md`.

#### Claude Code
- **Q2 2026 sold-price tier refresh** (`services/gemini.ts:98–128`; baselines `services/gemini.pricing-baseline-2026-04-30.md` + `-pass2.md`) — Major prompt update from Claude-for-Chrome research across Depop/eBay/Poshmark sold listings (eBay 2x trust weighting, US-only, 0–30d vs 30–90d bucketing, condition-normalized, ~1800 samples across 18 tiers). NEW tiers: Y2K viral (Juicy NWT tracksuit $55–$130, vintage USA-made $100–$200; Von Dutch hat $20–$65; Ed Hardy tee $25–$95), factory sneakers (Generic $20–$60 → Premium $30–$80 → Hyped Jordan/Dunk/Yeezy $40–$250 → Designer collab Travis Scott $250–$400, NB 2002R Salehe Bembury $108–$253), vintage non-denim Americana (Pendleton/Filson/LL Bean/Eddie Bauer/Woolrich), knit sets/loungewear, boots (combat/western/knee-high), hats (vintage trucker / branded dad cap / beanie). Refined: Carhartt women's WJ130/WJ141 rare-color callout $100–$350 (vs $25–$80 standard), Madewell premium-within-mall, premium denim cut differentiation (skinny FALLING, flare/wide-leg SPIKING), Y2K embellished spike (Diesel +45–58%, Rock Revival/Miss Me +27%), vintage Levi's Big E selvedge raw $300–$590, sterling silver sub-tiered, Reformation $80–$195, LV Speedy 25 $500–$1200, Coach NWT $105–$385. New `Q2 2026 ACTIVE SPIKES` line (crochet/cargo/boho-romantic/Y2K maximalist via Euphoria S3 collab, +30–50%). Three rollback levels via the two baseline files. Quarterly refresh scheduled as routine `trig_01384uFw9pzjb3mENCLm9wSM` firing 2026-07-30 — opens placeholder PR with v2 research prompt embedded, does not touch `gemini.ts` directly.
- **Tier-aware rounding for scan green price tag** (`utils/currency.ts`, `services/gemini.ts:534`) — Added `roundDisplayPrice(value)` helper: <$200 → nearest $5, <$500 → $10, <$1000 → $25, ≥$1000 → $50; floors at $5 so positive inputs never display $0. Applied at source in `runScanPipeline` so green tag display, persisted `Item.resale`, and saved-for-later restore all stay in sync. Fixes awkward midpoints ($63 → $65; $318 → $320).
- **Background scan perceived-seamless retry** (`app/(tabs)/scan.tsx`, `POST_LAUNCH.md`) — When iOS suspends mid-scan network, the catch now suppresses the toast (gated on `pendingRetryRef.current`) and the finally keeps `setScanning(true)`. AppState handler waits on `!scanningRef.current` before clearing the retry flag (so the original promise can settle first); finally fires retry via `queueMicrotask` if app is already foreground. Net: spinner stays continuous through background→foreground, no false toast, retry threads seamlessly. Doesn't actually complete the scan in background — real fix needs a `UIApplication.beginBackgroundTask` Expo native module + prebuild, deferred to POST_LAUNCH. Updated CLAUDE.md "Background scan" Key Decision: real fix is `beginBackgroundTask`, NOT "Background Fetch entitlement" (different iOS API for periodic fetches — earlier note was wrong).
- **Before/After photo recognition** (`services/gemini.ts` prompt, `types/inventory.ts`, `app/(tabs)/scan.tsx`, `app/detail.tsx`, memory `feedback_photo_position_agnostic.md`) — Position-agnostic detection across the 1–5 staged photo set. Photo 1 is always the cover (reseller convention); slots 2–5 independently classified per-photo as "(a) angle of finished item" (default) or "(b) before state". ≥1 (b) → `beforeAfterDetected: true` + `isCustom` forced + altered-base pricing tier. Prompt strongly defaults to (a) ("Most scans will NOT have a 'before' photo. THIS IS THE EXPECTED, COMMON CASE.") with explicit visual cue list (color/pattern shift, factory tags vs custom work, styling) and false-positive guard. Field persisted on both `ScanScenario` and `ItemScanSnapshot`; carries through `RESCAN_CORRECTION_SUFFIX` so rescans don't lose the verdict. UI: "Before / After" pill (swap-horizontal-outline icon) renders alongside Handmade pill on the same line via `staticPillRow` (scan.tsx) / `insightsCustomPillWrap` (detail.tsx) — both flexDirection: row, gap: 8.
- **Profit subtext hidden on grid cards** (`app/(tabs)/index.tsx`) — Commented out (not deleted) `{formatMoneyWithSign(estProfit)} profit` / `Sold $X` on Flips item cards and ` · $X profit` caption on Hauls cards (both with-title and without-title variants). Outer condition for caption-without-title narrowed from `(!!storesLabel || haul.profit > 0)` to `!!storesLabel` so empty caption lines don't render when only profit was the truthy flag. `profitLabel` variable still computed for one-step uncomment. Stats strip on Profile screen still shows totals.

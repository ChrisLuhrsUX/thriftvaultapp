...
- **Pricing-tier drift watch routine**, Weekly remote agent (Mon 9am ET, `trig_0113xK23HSeSpB46ySDYJtBF`) spot-checks 10 pricing tiers per week (6 HOT + 3 WARM + 1 COOL) drawn from `PRICING_TIERS.md` via weighted-by-volatility rotation. Routine fetches the registry at run time from raw.githubusercontent.com and opens a PR with `pricing-drift-reports/YYYY-MM-DD.md` per format spec at `pricing-drift-reports/README.md`. HOT pool ~12 tiers cycles every 2 weeks, WARM ~24 every 8 weeks, COOL ~49 every ~11 months. Does NOT modify `services/gemini.ts`, user reviews report and applies band edits manually. Bridges quarterly full refresh (next 2026-07-30). Routine prompt lives in schedule UI, registry + format spec live in repo and are PR-editable.

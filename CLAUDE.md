...
### Session, 2026-05-17
- **NEW pricing tier 086, Knit Bodycon / Off-Shoulder Mini Dresses** (`services/gemini.ts:259`, `PRICING_TIERS.md`). Trendy unbranded ribbed-knit silhouette (off-shoulder/square-neck/scoop/cowl/halter in olive/sage/cream/etc.) was falling into Unknown/Unbranded $10-$30 commodity floor; real Depop/Poshmark/eBay comps $18-$35. New tier anchors unbranded $20-$32, branded mid-tier (Princess Polly/PLT/Lulus/Boohoo/ASOS dress range/F21 dress) $25-$40. Q2 spring-knits spike baked in to prevent compound boost. Aritzia/Free People/Anthropologie/Reformation kept on their existing contemporary tier. **Pattern lesson: trending-color + trending-silhouette unbranded combos need their own tier; the commodity-floor catch-all underprices anything currently moving on TikTok/IG.**
- **NEW pricing tier 087, Modern Multi-Function Record-Player Cabinets + sibling PARTICLEBOARD MASQUERADE MODERN REPRO EXEMPTION** (`services/gemini.ts:318`, `:493`, `PRICING_TIERS.md`). Crosley/Victrola/Innovative Technology/Wockoder/ION Audio/Jensen repro units ($80-$200 retail new at Walmart/Target/Costco) were pricing $60-$120 via Generic vintage no-brand $40-$300 tier; real used comps $30-$75. New tier explicit tells (any ONE confirms): CD slot, cassette deck, AUX/USB/Bluetooth, plastic platter, mesh-grille integrated speakers, decorative-only radio dial face, 14-24" footprint, MDF veneer. Explicit "do NOT route through Generic vintage / Vintage American mid-tier / antique" instruction. Authentic 60s-70s console stereos kept separate at $150-$800+ via positive tells (4-6ft cabinet, real solid wood, die-cast platter, no modern features, manufacturer plate, 100+lbs). PARTICLEBOARD MASQUERADE flag suppresses for matching units because new tier already prices for MDF/veneer. **Pattern lesson 1: visual styling that references a vintage era is NOT vintage evidence; require positive vintage tells. Pattern lesson 2: red flags should suppress when the price tier already accounts for the substrate concern, otherwise the warning becomes noise.**
- **CAPTION/OVERLAY EXEMPTION on isCustom + beforeAfterDetected** (`services/gemini.ts:126`, `:818`). TikTok narrative captions ("THEN I DID", "BEFORE / AFTER", "FIRST I... THEN I", "watch me transform", "from this to this", "POV: I made") were tripping `beforeAfterDetected = true` → cascading `isCustom = true` via line 822 → routing clean factory dresses through HANDMADE DRESS EXCEPTION ($80 vs real ~$25). Inserted CAPTION/OVERLAY EXEMPTION at TOP of isCustom rule (rejects modification claims without physical evidence in garment) + expanded multi-photo classifier's caption bullet with explicit temporal-narrative examples and closing rule: if all photos show matching construction and only "before" signal is text, classify all as (a) multi-angle and set `beforeAfterDetected = false`. Screenshot memory broadened from "screenshots never drive RED FLAGS" to "screenshots never drive ANY classification" (red flags, isCustom, beforeAfterDetected, future fields). **Pattern lesson: every new classification signal must include caption-exemption carve-out at the TOP of the rule, not as a bottom-of-list disclaimer; bottom-of-list disclaimers get ignored by the model when the bias direction at the top is strong.**
- **`<Button>` component + 9 CTA replacements + Flips/Closet card unification + touch-target HIG pass** (`components/Button.tsx` NEW, `app/(tabs)/index.tsx`, `app/(tabs)/profile.tsx`, `app/detail.tsx`, `components/PaywallModal.tsx`). Button: variants primary/secondary/ghost + sizes md (48px) / lg (56px) + loading + icon left/right, all theme tokens (radius.sm + spacing.xl/xxl + vintageBlueDark). Replaced markSoldBtn/errorBtn/addToClosetBtn/newHaulBtn/2×emptyBtn/emptyBtnSecondary(ghost)/upgradeBtn/PaywallModal cta. Removed ~80 lines orphan + dead-code styles (photoBtn/removePhotoBadge/collageRemoveBtn/addBtn). All hardcoded radii (24, 28) and one-off heights gone. Card unification: Flips/Closet cards gained outer border (surfaceVariant 1px), `cardImageBlock` wrapper, `cardFooter` wrapper with lavender border-top separator + sm/xs/sm padding, 2-line truncated name `flex: 1` left + right-aligned profit-colored price grounded `alignItems: 'flex-end'`; matches Haul `collageCell` structurally. Touch targets: filter chips + Add photos link got `hitSlop {top:5, bottom:5}` (34px → 44px hit area, visual unchanged); store picker chips got `minHeight: theme.minTouchTargetSize` (wrap layout, hitSlop would overlap rows). **Pattern lesson 1: same-intent CTAs need component-level enforcement; per-screen styles drift even when devs reach for the same theme tokens. Pattern lesson 2: shadow alone isn't enough to define card edge on cream bg; surface-on-cream dissolves without a border. Pattern lesson 3: 2-line truncated name + right-aligned price reads as marketplace standard (Depop/Poshmark/Etsy); stacked name-then-price feels like a tag stack and less polished.**
- **BOIR exempt, NOT required to file** (project memory updated). FinCEN interim final rule 2025-03-26 narrowed "reporting company" to foreign-formed entities only. ThriftVault LLC (TN-formed) is exempt. No deadline applies. Re-verify when FinCEN finalizes new permanent rule (expected 2026). Apple org-conversion reply sent 5/16 with live `thriftvaultapp.com` domain; awaiting Apple response. AI detection `__DEV__` log instrumentation + `EXPO_PUBLIC_FORCE_CLAUDE_SCAN` bypass stripped (5/14 parked debug plan abandoned).

### Session, 2026-05-16
- **MULTI-ZONE PRINTED TEXTILES carve-out on ALL-OVER DIGITAL PRINT** (`services/gemini.ts:423`). False-positive red flag on a Y2K mesh maxi kaftan (TikTok-shared thrift find) showing animal print + tribal geometric + abstract painterly zones printed in adjacent "patchwork-style" panels on a single continuous fabric. Existing exemptions covered single-pattern textiles (animal print alone, paisley alone, etc.) and actual multi-fabric patchwork construction, but not the decades-old print tradition where multiple textile repeats are mosaic'd into one printed design (Sky to Moon / Charlotte Russe / Forever 21 mall-tier 2005-2015 boho/festival/Y2K mesh kaftans, sarongs, beach coverups). New exemption explicitly requires: zones filled with textile repeats (no pictorial subjects), one continuous polyester/mesh/jersey panel with no seams, V-neck or deep-V maxi silhouette tells. Carve-out does NOT cover genuinely pictorial zones (characters, scenes, slogans, photorealistic imagery), those still flag. Sits between the existing multi-panel construction exemption and the lolita pictorial exemption. **Pattern lesson: textile-exemption lists need a "multi-pattern mosaic on single fabric" clause to match decades-old printed-patchwork aesthetic; treating each zone individually as exempt isn't enough because the model reads the combination as pictorial collage.**
- **Pricing-drift watch upgraded from 14 fixed tiers to 85-tier registry with weighted rotation** (`PRICING_TIERS.md`, `pricing-drift-reports/README.md`, routine prompt in `trig_0113xK23HSeSpB46ySDYJtBF`). Coverage gap closed: 81+ distinct tiers in `services/gemini.ts` previously got spot-checked only at quarterly refreshes (next 2026-07-30); now HOT 12 cycles every 2 weeks, WARM 24 every 8 weeks, COOL 49 every ~11 months, deterministic ISO-week hash drives slice selection so the routine is stateless. Registry markdown table has per-row Depop/Poshmark/eBay queries (furniture rows substitute Chairish/1stDibs/Etsy via `(Chairish)` cell prefix); routine prompt fetches both `PRICING_TIERS.md` and `pricing-drift-reports/README.md` via WebFetch each Monday, computes the week's 10-tier slice with `HOT_POOL[(WEEK*6) mod len..][:6]` plus parallel WARM/COOL slices, fills the report template, opens PR. User edits tier registry via PR (no schedule UI changes needed for adds/removes/promotions). Code-level clamps (DIY Distressed Denim, BOOST STACKING, Confidence Downgrade, Crochet text-detection) live in the registry as `code_clamp` rows with platform queries = `n/a`; routine switches to narrative sanity-check format for those rows. Verdict thresholds: 15% out-of-band median OR 2+ platforms outside = drift; weighted eBay 2x; suggested new band rounded per `roundDisplayPrice`. **Pattern lesson: routine reference data that changes frequently belongs in the repo (PR-editable, gitted, diffable); routine prompt that changes rarely stays in the schedule UI. Inverts the usual code-in-repo / prompt-in-config split because the agent is remote and its prompt is the harder thing to version.**

### Session, 2026-05-15
- **Red Flag banner split into verification vs counterfeit framing** (`services/gemini.ts:54–74` `classifyRedFlags`, `app/(tabs)/scan.tsx:219–339`, `app/detail.tsx:39,764–769,1143–1207`). Prefix-matches `redFlags` strings against 4 furniture verification prefixes (`Verify solid wood vs printed-veneer`, `Smell-verify`, `Inspect seams and joints`, `Structural damage visible`); all-verification renders teal "Worth verifying" + softer "Inspect in person before paying high prices." + single "Got it" dismiss. Mixed/counterfeit (AI artwork, stock-photo sentinel, MCM KNOCKOFF, bag/sneaker/jewelry masquerade) keeps existing red framing + Yes/No. `Got it` reuses the existing false-alarm handler so dismissal persists the same way (`{redFlagPrompt:true,redFlagBanner:true}`). No schema/migration, prompt's flag templates are stable enough that prefix matching is durable. **Pattern lesson: string-mixed array types should classify in UI via prefix matching, not require schema/migration when the producer (prompt) controls phrasing.**
- **PARTICLEBOARD MASQUERADE substrate-proof gate** (`services/gemini.ts:466`). False-positive on 4-photo mahogany→sage-green dresser refurb. Inserted positive-identification precondition: if `beforeAfterDetected = true` AND any non-cover photo shows raw wood grain (sanded surfaces, unfinished drawer interiors, exposed end-grain, unpainted sections, dovetail/mortise joinery), substrate proof is definitive, SUPPRESS regardless of cover finish. Mirrors the existing MCM bypass pattern (positive identification beats speculative flag). **Pattern lesson: prompt exemption phrased "don't speculate" is weaker than "use X as positive proof"; invert directives where possible.**
- **AI-GENERATED ARTWORK false positive on Tumblr-shared Y2K checkerboard pants** (`services/gemini.ts:426,429,434,446`). 4 prompt edits: (1) line 434 garbled-text tell gains UI/overlay exemption mirroring the AI-GENERATED PHOTO branch (UI overlays, captions, watermarks, social-media re-share text, reblog/repost indicators, hashtags, usernames, foreign-script overlays, screenshot chrome); (2) line 426 PRECONDITION now requires positive identification ("name the design out loud, a [character/scene/slogan/etc.] is printed on the [garment area]"), if only candidate is geometric repeat/color block/textile pattern the precondition fails; (3) line 429 "checks" exemption broadened to neon/high-contrast/two-color/Vans-style/racing/Y2K checkerboard variants; (4) line 446 platform list gained Tumblr (reblog/repost/reposted indicators and dashboard chrome). **Pattern lesson: sister branches with parallel tells need synchronized exemptions; the artwork branch was missing the photo branch's screenshot/UI carve-out, so social re-share chrome tripped the garbled-text tell.**
- **Y2K alt-fashion non-denim pants tier with Depop/Poshmark/eBay sold-comp anchors** (`services/gemini.ts`, new tier between Y2K viral brands and Modern Levi's). Targets Tripp NYC / Lip Service / Hot Topic Morbid line / modern Y2K-revival twill/polyester flares with subculture detailing (checkerboard waistbands, ribbon-laced sides, oversized D-rings, chain trim, parachute/cargo straps, contrast piping). Sold-comp anchors per-platform, last 90 days: unbranded modern Depop $20–$45 / Poshmark $15–$40 / eBay $20–$50 (default $25–$55 lower-middle); authenticated vintage Tripp/Lip Service with visible brand tag $40–$90 Depop / $35–$75 Poshmark / $50–$120 eBay. Tier explicitly blocks routing through Premium denim flare SPIKING, Authenticated Y2K premium denim, AND HANDMADE PANTS EXCEPTION (the last clause was added in a second iteration after handmade path bypassed the first version of the tier). Q2 2026 Y2K spike already factored in, do NOT compound. **Pattern lesson: pricing tiers need explicit "do NOT route through X" clauses for each adjacent path or the model will bypass via the cheapest semantic route.**
- **isCustom carve-out for factory subculture detailing** (`services/gemini.ts:137`). "Waistbands added or removed" tell tightened to require visible modification evidence (raw cut edges at the waistband join, mismatched thread color/weight, hand-finished topstitching, puckered attachment). EXCLUSION block added for factory subculture detailing on Y2K alt-fashion pants/skirts: contrast/decorative waistbands (checkerboard, grosgrain, jacquard, studded, eyelet-laced), oversized D-rings, chain trim, ribbon-laced side panels, parachute/cargo straps, contrast piping are factory-manufactured details with clean factory stitching at the join, NOT isCustom signals. Belt-and-suspenders carve-out at the top of `HANDMADE PANTS EXCEPTION` (`services/gemini.ts:241`) bails out to factory pricing if isCustom still fires on these items. **Pattern lesson: the line 125 "when in doubt set isCustom = true" directive biases the model toward over-flagging, so specific exclusions must name the bias to push back.**
- **Duplicate-detection stop-words extended + threshold bumped** (`app/(tabs)/scan.tsx:100–112, 143`). `DUPLICATE_STOP_WORDS` gained era markers (vintage/retro/y2k/2k/deadstock/og/90s/80s/70s/60s/2000s/nineties/eighties/seventies/sixties) and condition tokens (condition/used/nwt). `DUPLICATE_SCORE_THRESHOLD` raised 0.55 → 0.60. Targets the 3-token era+color generic-overlap false-positive shape ("Vintage Y2K Black Flare Pants" vs "Vintage Y2K Black Mini Skirt" was clearing the floor via vintage+y2k+black overlap; now drops to 1 shared token after stop-word stripping, existing `!brandMatch && matchedTokens.size < 3` floor clamps to 0.40). Image-size byte-exact fallback (0.99) and brand-anchored matches unaffected.
- **Removed `LEGACY_DEMO_ITEM_NAMES` migration filter** (`constants/seedItems.ts`, `context/InventoryContext.tsx:1,129–130`). Dead code, demo items from earlier dev builds never reached real users (app is pre-launch). Tiny startup win: 291-item inventory no longer `Set.has()`-checks against 13 demo names on every hydration. `DEFAULT_ITEM_PLACEHOLDER_IMAGE` retained (still used at `scan.tsx:1583`).
- **Detail-page scan card asymmetry confirmed intentional** (chat-only, no code). Scan card on `scan.tsx` shows a bold single recommended price `formatMoney(scenario.suggestedResale)` + range. Detail page shows the range on the snapshot card and auto-fills the single recommended price into the editable `item.resale` field. Two numbers (range + editable resale), not three (no frozen original recommendation). Adding `suggestedResale` to `ItemScanSnapshot` would create three competing prices and a stale frozen number once the user edits resale.
- **Cloudflare Web Analytics chosen for landing page** (planned, not wired). Free drop-in `<script>` beacon, no cookies, no consent banner, no DNS migration. Setup is sign up → add domain → paste snippet in `index.html` `<head>` → wait ~24h. GoatCounter is alternative if user prefers no Cloudflare account. Privacy policy needs a one-line disclosure once wired ("aggregate visit counts via Cloudflare Web Analytics, no cookies or personal data").

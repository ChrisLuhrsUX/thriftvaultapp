...
- **Logo references reverted to v1 hanger** (`app.json`, `components/WebSidebar.tsx`) — chose hanger logo (`thriftvault_logo.jpg`) over v2 camera-frame for scan background and rolled back the v2.png swap across icon/splash/Android adaptive/web favicon. Reasoning: camera-frame version creates metaphor collision (literal camera *inside* the scan card while shutter button overlays *on top* of it); hanger composition centers cleanly behind shutter and the "THRIFT VAULT" tag reinforces brand. Native icon/splash won't update until next prebuild. App Store icon export still pending (1024×1024 opaque RGB requirement).

...
- **Em dash sweep + memory** (`feedback_no_em_dashes.md`). User hates em dashes. Swept 32 tracked files for literal `—` (replaced with commas, en dashes in `$130–$160` ranges preserved) plus 4 `&mdash;` HTML entities in `index.html` that the literal-character grep missed. New memory entry indexed, includes the HTML-entity gotcha for future sweeps.

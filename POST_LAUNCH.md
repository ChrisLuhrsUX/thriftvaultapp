...
- [ ] **Condition-weighted headline price** — currently the displayed point estimate defaults to the midpoint of the resale range (50%). Real sold-price distributions are right-skewed: used-good items typically clear at ~35–40% of the range, NWT/like-new at 60–70%, midpoint only fits "average condition, no signal." Bias the headline by condition (or simply lower the default to ~40%) so the user-facing number lands closer to actual median sold price. Pre-launch decision: park because there's no real-user signal yet on whether midpoint feels high; a 20-scan user feedback loop will tell us more than statistical theory. Pricing tier work in `services/gemini.ts` is unaffected — this is purely a display bias in the scan result card and item detail header.
...
Same problem as furniture: clothing-tuned prompt mis-prices pottery, ceramics, glass, woodturning, metalwork, and other small handcraft. Market dynamics differ — maker/studio matters enormously (signed studio pottery, named potters like Lucie Rie/Hans Coper/Peter Voulkos on the very high end; mid-century named studios like Heath, Bauer, Roseville, McCoy, Frankoma in the middle; unsigned hobbyist work at the bottom), maker's marks/signatures/stamps drive 5–50× multipliers, condition penalties are brutal (hairline cracks, chips, crazing, repaired breaks tank value), and form/glaze rarity matters. Platform context shifts too (eBay and Etsy for studio pottery, 1stDibs/Chairish for high-end signed pieces, local for heavy/fragile shipping risk). Fix: add a pottery/craft maker-tier block to `PROMPT`, `ItemCategory = 'pottery'` (or broader `'craft'`), explicit signature/maker's-mark detection guidance, and a condition block tuned to ceramic damage (chips, cracks, crazing, glaze loss, repairs). Needs its own `POTTERY EXCEPTION` so the handmade labor formula doesn't absurdly price a $20 hobbyist mug like a custom commission.

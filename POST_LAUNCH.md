...
**Partially addressed 2026-05-18** with Vintage Tableware tier (088) covering Pyrex / Fiestaware / Wedgwood / Spode / Noritake / Roseville / McCoy / Royal Doulton / Scandinavian MCM / Boleslawiec / Italian majolica, plus TABLEWARE BACKSTAMP & ERA HARD RULE gating vintage premium pricing on visible maker's mark + post-1986 Fiestaware trap-color list keeping reissues out of vintage tier + condition penalties (chip/crack/repair, crazing, glaze loss, missing set pieces). Remaining gaps: (1) signed grail-tier studio pottery (Lucie Rie, Hans Coper, Peter Voulkos, Beatrice Wood, Toshiko Takaezu) needs its own band beyond the current $40–$300 signed studio pottery ceiling; (2) dedicated `ItemCategory = 'pottery'` or broader `'homewares'` for vault grouping (current routing reuses `'furniture'`); (3) photo-detection clamp for hairline cracks / crazing / repair (currently prompt-only visual inspection); (4) the handmade labor formula still applies if `isCustom=true` is detected on a one-off studio piece, needs explicit `POTTERY EXCEPTION` so a $20 hobbyist mug isn't priced like a custom commission.
